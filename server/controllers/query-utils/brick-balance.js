...
const bbWhere = (legoSetNumbers) => {
    return `
      (
        ${legoSetNumbers.map((setNum, index) => {
        return index === 0 ? `s${setNum}.quantity IS NOT NULL` : `or s${setNum}.quantity IS NOT NULL`
    }).join(' ')
        }
      ) and (          
        ${legoSetNumbers.map((setNum, index) => {
            return index === 0 ? `s${setNum}.lego_set_id = ?` : `or s${setNum}.lego_set_id = ?`
        }).join(' ')
        }
      )
    `;
...
const bbSelect = (legoSetNumbers) => {
    return `
        ...
        (
            ...
            ${legoSetNumbers.map((setNum, index) => {
            return index === 0 ? `ifnull(s${setNum}.quantity, 0)` : `+ ifnull(s${setNum}.quantity, 0)`
        }).join(' ')
